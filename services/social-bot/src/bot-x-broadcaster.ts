...
export class HikariXBroadcaster {
  ...
  public formatTvlMilestoneTweet(milestoneName: string, metrics: ProtocolMetrics): string {
    return [
      ...
      "",
      `📊 EarnXLM TVL: ${metrics.tvlXlm.toLocaleString()} XLM (${metrics.apyXlm.toFixed(1)}% APY)`,
      `💵 EarnUSD TVL: $${metrics.tvlUsd.toLocaleString()} USD (${metrics.apyUsd.toFixed(1)}% APY)`,
  ...
  public formatAlphaRecapTweet(topStrategy: string, weeklyApy: string, totalCompounded: string): string {
    return [
      ...
      "",
      "Hikari Protocol's autonomous yield agents have executed 168 (Simulated) successful auto-compounds this week.",
      "",
